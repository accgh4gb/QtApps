    ...
    ui(new Ui::MainWindow)
{
    ...
    for(int i = 0; i < list.size(); i++)
        ui->currentDisk->addItem(list.at(i).path());
}
...
void MainWindow::on_insertData_clicked()
{
    ...
    cursor.insertText(' ' + date.toString("dd:MM:yyyy") + ' ');
}

void MainWindow::on_insertTime_clicked()
{
    QTime time = QTime::currentTime();
    QTextCursor cursor = pText->textCursor();
    pText->setTextCursor(cursor);
    cursor.insertText(' ' + time.toString("hh:mm") + ' ');
}

void MainWindow::on_searchButton_clicked()
{
    QStringList fileNames = {ui->searchLine->text()};
    path = ui->currentDisk->currentText();
    path += QDir::cleanPath("/");
    QDir currentDir = QDir(path);
    thread = new myThread(fileNames, path, currentDir);
